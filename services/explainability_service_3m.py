...
class ExplainabilityService3M:
    ...
    def initialize_explainer(self, seq_length=60, num_background_samples=100):
        ...
        try:
            ...
            from services.forecast_service_3m import model_3m, scaler_3m, historical_data_3m
            
            if model_3m is None or scaler_3m is None or historical_data_3m is None:
                raise RuntimeError("Model, scaler, or historical data not loaded")
            
            recent_data = historical_data_3m.tail(num_background_samples + seq_length)
            df = recent_data.copy()
            df = df.dropna()
            ...
            print(f"✅ SHAP explainer (3M) initialized with {len(self.background_data)} background samples")
            return True
    ...
    def get_shap_values(self, features: InputFeatures3M, seq_length=60) -> Dict:
    ...
    def get_eli5_feature_importance(self, features: InputFeatures3M, seq_length=60) -> Dict:
    ...
    def get_combined_explanation(self, features: InputFeatures3M, seq_length=60) -> Dict:
        ...
        try:
            ...
            print(f"❌ Error generating combined explanation (3M): {e}")
            raise RuntimeError(f"Explanation generation failed: {e}")

# Global instance
explainability_service_3m = ExplainabilityService3M()

def get_explanation_3m(features: InputFeatures3M) -> Dict:
    """Main function to get model explanation"""
    return explainability_service_3m.get_combined_explanation(features)
