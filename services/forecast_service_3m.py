...
MODEL_3M_PATH = "ml_models/3m/lstm_transformer_model_3m.keras"
SCALER_3M_PATH = "ml_models/3m/lstm_transformer_scaler_3m.pkl"
...
def load_model_3m():
    """Load the LSTM+Transformer 3-month recession prediction model"""
    global model_3m
    try: 
        ...
        print("✅ LSTM+Transformer 3M model loaded successfully")
        return True
    except Exception as e:
        print(f"❌ Error loading LSTM+Transformer 3M model: {e}")
        model_3m = None
        return False

def load_scaler_3m():
    """Load the scaler for LSTM+Transformer 3-month model preprocessing"""
    global scaler_3m
    try:
        scaler_3m = joblib.load(SCALER_3M_PATH)
        print("✅ LSTM+Transformer 3M scaler loaded successfully")
        return True
    except Exception as e:
        print(f"❌ Error loading LSTM+Transformer 3M scaler: {e}")
        scaler_3m = None
        return False
...
def preprocess_features_3m(features: InputFeatures3M, lookback_points=120) -> tuple:
    """
    Preprocess input features for LSTM+Transformer 3-month model
...
def initialize_3m_service():
    """Initialize the LSTM+Transformer 3 months forecasting service"""
    ...
    print("🚀 Initializing LSTM+Transformer 3M forecasting service...")
    ...
    if model_loaded and scaler_loaded and data_loaded:
        print("✅ LSTM+Transformer 3M service initialized successfully")
        _service_initialized_3m = True
        return True
    else:
        print("❌ LSTM+Transformer 3M service initialization failed")
        _service_initialized_3m = False
        return False

def predict_3m(features: InputFeatures3M, threshold=0.72) -> ForecastResponse3M:
    """
    Make 3-month recession probability prediction using LSTM+Transformer model
    
    Key changes from previous model:
    - Uses sequence length of 12 (vs 60)
    - Uses threshold of 0.72 (vs 0.3)
    - Different model architecture (LSTM+Transformer)
    """
    if not _service_initialized_3m:
        if not initialize_3m_service():
            raise RuntimeError("Failed to initialize LSTM+Transformer 3M forecasting service")
    
    if model_3m is None or scaler_3m is None:
        raise RuntimeError("LSTM+Transformer 3M model or scaler is not loaded")
    ...
    try: 
        ...
        raise RuntimeError(f"LSTM+Transformer prediction failed: {e}")   

def get_model_info_3m() -> ModelStatus3M:
    """Get information about the LSTM+Transformer 3-month model"""
    return ModelStatus3M(
        ...
        model_architecture="LSTM+Transformer Hybrid",
        sequence_length=MODEL_CONFIG['seq_length'],
        threshold=0.72
    )

def test_prediction_3m():
    """Test function to verify the LSTM+Transformer service works"""
    # Create test input using the same data from 1M model for consistency
    ...
    try:
        ...
        print("=== LSTM+TRANSFORMER 3M RECESSION PREDICTION RESULTS ===")
        ...
        print(f"❌ LSTM+Transformer test prediction failed: {e}")
        return None


if __name__ == "__main__":
    # Test the service
    print("Testing LSTM+Transformer 3M forecasting service...")
